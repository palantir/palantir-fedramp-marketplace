...
Do not include `metadata.lastUpdated` in YAML. Generation sets it to the current UTC time when a package's data changes, and preserves it otherwise. YAML comments, formatting, and key order do not count as data changes. Template-only edits regenerate Markdown without changing the JSON timestamp. Each package is handled independently, and repeated generation leaves unchanged files untouched.
...
- `data/*.yaml` contains the maintained package information, without `metadata.lastUpdated`.
